...
from .models import Dealer, Vehicles, Area
...
def auth_view(request):
    if request.user.is_authenticated:
        ...
        try:
            car_dealer = Dealer.objects.get(car_dealer=user)
        except:
            car_dealer = None
...
def registration(request):
    ...
    try:
        ...
        area = None
    if area is not None:
        car_dealer = Dealer(car_dealer=user, mobile=mobile, area=area)
    else:
...
def add_vehicle(request):
    ...
    cd = Dealer.objects.get(car_dealer=request.user)
    city = request.POST['city']
    city = city.lower()
    pincode = request.POST['pincode']
